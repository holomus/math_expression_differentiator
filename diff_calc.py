...
class mathParser(Parser):
    ...
    def program(self, p):
        return self.Program(p.optfuncs, p.optdiffarg, p.optexprs)
    ...
    @_('func funcs')
    def funcs(self, p):
        return self.Funcs(p.func, p.funcs)

    @_('func')
    def funcs(self, p):
        return self.Funcs(p.func, None)
    ...
    def func(self, p):
        if p.ID0 in self.funcs:
            ...
            self.funcs[self.ID0] = self.Func(p.ID0, p.ID1, p.expr)
            return self.funcs[self.ID0]

    @_('"{" DIFF ID "}"')
    def diffarg(self, p):
        return self.DIFFARG(p.ID)

    @_('"{" expr "}" exprs')
    def exprs(self, p):
        return self.Exprs(p.expr, p.exprs)

    @_('expr')
    def exprs(self, p):
        return self.Exprs(p.expr, None)
    ...
    @_('expr PLUS expr',
       ...
       'expr POW expr')
    def expr(self, p):
        return self.BinOp(p[1], p.expr0, p.expr1, self.funcs)

    @_('MINUS expr %prec UMINUS',
       'PLUS  expr %prec UPLUS')
    def expr(self, p):
        return self.UnOp(p[0], p.expr, self.funcs)
    ...
    @_('SIN  expr',
       ...
       'SQRT expr')
    def expr(self, p):
        return self.FuncOp(p[0], p.expr, self.funcs)
    ...
    def expr(self, p):
        if p.ID not in self.funcs:
            ...
            return self.FuncOp(p.ID, p.expr, self.funcs)
    ...
    @_('ID')
    def expr(self, p):
        return self.ID(p.ID, self.funcs)

    @_('NUMBER')
    def expr(self, p):
        return self.Number(p.NUMBER, self.funcs)
